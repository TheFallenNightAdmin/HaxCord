...
const getJSON = phin.defaults({
    method: "GET",
    parse: "json",
    headers: {"User-Agent": "HaxCord Installer"},
    followRedirects: true
});

/* eslint-disable no-console */
export default async function () {
    const downloadUrl = "https://api.github.com/repos/TheFallenNightAdmin/HaxCord/releases";
    console.info(`HaxCord Installer ${version}`);
    ...
    try {
        ...
        if (semverGreaterThan(latestVersion, version)) {
            ...
            const result = await dialog.showMessageBox({
                ...
                message: `A new version of the HaxCord installer is available. Click "Download" to download the newest version.`,
